...
class SquatAnalyzer2(private val context: Context,
    ...
    // TensorFlow Lite for 30 features
    private var interpreter: Interpreter? = null
    private val inputBuffer: ByteBuffer = ByteBuffer.allocateDirect(1 * 60 * 30 * 4).apply {
    ...
    private fun loadModel() {
        try {
            val modelBuffer = FileUtil.loadMappedFile(context, "squat_manual_lstm_30_features.tflite")
            interpreter = Interpreter(modelBuffer)
            Log.d(TAG, "TFLite model with 30 features loaded successfully!")
        } catch (e: Exception) {
    ...
    private fun normalizeLandmarks(result: PoseLandmarkerResult, refMidHip: FloatArray, refFemurLen: Float): FloatArray? {
        try {
            ...
            val leftFoot = points["LEFT_FOOT"] ?: return null
            val rightHeel = points["RIGHT_HEEL"] ?: return null
            ...
            // Additional heel features
            val leftHeelLift = leftHeel[1] - leftFoot[1]
            val rightHeelLift = rightHeel[1] - leftFoot[1]
            val heelLiftIndicator = (leftHeelLift + rightHeelLift) / 2f
            val heelInstability = abs(leftHeel[1] - rightHeel[1])

            // Build feature vector (24 coordinates + 6 features = 30 total)
            ...
            features.addAll(listOf(hipHeelDist, hipAnkleDist, shoulderHeelDist, shoulderGap,
                heelLiftIndicator, heelInstability))
    ...
    private fun predictWithTFLite(inputData: Array<FloatArray>): String? {
        ...
        try {
            Log.d(TAG, "[TFLite] Starting prediction with shape: ${inputData.size}x${inputData[0].size}")
            ...
            Log.d(TAG, "[TFLite] Input buffer filled with $valueCount values (expecting ${60 * 30} for 30 features)")

            interpreter.run(inputBuffer, outputBuffer)
            Log.d(TAG, "[TFLite] Inference completed successfully")

            val confidences = outputBuffer[0]
            Log.d(TAG, "[TFLite] Raw output: [${confidences.joinToString(", ") { "%.3f".format(it) }}]")
            ...
            Log.d(TAG, "[TFLite] Prediction: $prediction (index: $maxIndex, confidence: ${String.format("%.3f", maxConfidence)})")
